...
// back to auth. Copied verbatim from auth/integration/authmw (stdlib-only) so the
// sales backend can accept the unified dashboard login token directly (one login,
// no token bridge). See internal/transport/http for how it's wired alongside the
// legacy native token.
package authmw
...
// acceptance simply stays off — the backend keeps using its native token only).
